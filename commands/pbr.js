...
module.exports = {
	...
	async execute(interaction) {
		...
		if (interaction.options.getSubcommandGroup(false) === "admin") {
			...
			if (!author.permissions.has([PermissionsBitField.Flags.Administrator])) {
				interaction.reply({content: "You do not have permission to run that command", ephemeral: true});
			} else {
				...
				await interaction.channel.send({"content": "Apply to PBR", components: [row]});
	...
	async onMessageInteraction(interaction) {
		...
		let userRoles = interaction.member.roles.cache.filter(role => role.name.startsWith("cyber paladin"));
		let quarters = userRoles.size;
		customURL = customURL.replace("{{DISCORD_USERNAME}}", encodeURIComponent(interaction.user.tag))
